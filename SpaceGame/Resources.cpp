...
Resources::Resources()
{
	...
	if (!m_SpriteSheets[5]->loadFromFile("eggspritesheet.png"))
		throw FileNotOpenEx();
	m_SpriteSheets.emplace_back(std::make_unique<sf::Texture>());
	if (!m_SpriteSheets[6]->loadFromFile("fork.png"))
		throw FileNotOpenEx();
	m_SpriteSheets.emplace_back(std::make_unique<sf::Texture>());
	if (!m_SpriteSheets[7]->loadFromFile("upgrade.png"))
		throw FileNotOpenEx();
	m_SpriteSheets.emplace_back(std::make_unique<sf::Texture>());
	if (!m_SpriteSheets[8]->loadFromFile("fork.png"))
		throw FileNotOpenEx();
	m_SpriteSheets.emplace_back(std::make_unique<sf::Texture>());
	...
	if (!m_SpriteSheets[11]->loadFromFile("shipExplosion.png"))
		throw FileNotOpenEx();
	m_SpriteSheets.emplace_back(std::make_unique<sf::Texture>());
	if (!m_SpriteSheets[12]->loadFromFile("bos.jpg"))
		throw FileNotOpenEx();
	m_SpriteSheets.emplace_back(std::make_unique<sf::Texture>());
